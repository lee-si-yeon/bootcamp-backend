...
public class CashCompleteOrderPaymentUseCase {
    ...
    public void handle(MarketOrderPaymentRequestedEvent event) {
        ...
        if (canPay) {
            customerWallet.debit(event.getOrder().getSalePrice(), CashLog.EventType.사용__주문결제, "Order", event.getOrder().getId());
            holdingWallet.credit(event.getOrder().getSalePrice(), CashLog.EventType.임시보관__주문결제, "Order", event.getOrder().getId());
